...
QMutex CollectionDB::m_nMutext;
static QMutex closeMutext;
using namespace BAE;
...
bool CollectionDB::insert(const QString &tableName, const QVariantMap &insertData)
{
    ...
    for (int i = 0; i < totalFields; ++i)
        strValues.append("?");
    QMutexLocker locker(&closeMutext);
...
bool CollectionDB::addTrack(const FMH::MODEL &track)
{
    ...
    QMutexLocker locker(&m_nMutext);
    bool rv = insert(TABLEMAP[BAE::TABLE::SOURCES], sourceMap);
    if(rv == false){
        qWarning() << "addTrack insert SOURCES fail";
    }
    ...
    rv = insert(TABLEMAP[TABLE::ARTISTS],artistMap);
    if(rv == false){
        qWarning() << "addTrack insert ARTISTS fail";
    }
    ...
    rv = insert(TABLEMAP[TABLE::ALBUMS],albumMap);
    if(rv == false){
        qWarning() << "addTrack insert ALBUMS fail";
    }
    ...
    QVariantMap trackMap {{FMH::MODEL_NAME[FMH::MODEL_KEY::URL], url},
                          ...
                          {FMH::MODEL_NAME[FMH::MODEL_KEY::RELEASEDATE], QString::number(QDateTime::currentMSecsSinceEpoch())},
                          {FMH::MODEL_NAME[FMH::MODEL_KEY::ADDDATE], QString::number(QDateTime::currentMSecsSinceEpoch())},
    ...
    rv = this->insert(BAE::TABLEMAP[BAE::TABLE::TRACKS], trackMap);
    if(rv == false){
        qWarning() << "addTrack insert TRACKS fail";
    }
    return rv;
}

bool CollectionDB::addVideo(const FMH::MODEL &video)
{
    auto url = video[FMH::MODEL_KEY::URL];
    auto title = video[FMH::MODEL_KEY::TITLE];
    auto genre = video[FMH::MODEL_KEY::GENRE];
    auto sourceUrl = video[FMH::MODEL_KEY::SOURCE];
    auto duration = video[FMH::MODEL_KEY::DURATION];

    QVariantMap videoMap {{FMH::MODEL_NAME[FMH::MODEL_KEY::URL], url},
                          {FMH::MODEL_NAME[FMH::MODEL_KEY::SOURCE], sourceUrl},
                          {FMH::MODEL_NAME[FMH::MODEL_KEY::TITLE], title},
                          {FMH::MODEL_NAME[FMH::MODEL_KEY::DURATION], duration},
                          {FMH::MODEL_NAME[FMH::MODEL_KEY::RELEASEDATE], QString::number(QDateTime::currentMSecsSinceEpoch())},
    ...
    QMutexLocker locker(&m_nMutext);
    return this->insert(BAE::TABLEMAP[BAE::TABLE::VIDEOS], videoMap);
}
...
bool CollectionDB::playedVideo(const QString &url, const int &increment)
{
    QMutexLocker locker(&m_nMutext);
    auto queryTxt = QString("UPDATE %1 SET %2 = %2 + %3 WHERE %4 = \"%5\"").arg(TABLEMAP[TABLE::VIDEOS],
            FMH::MODEL_NAME[FMH::MODEL_KEY::COUNT],
            QString::number(increment),
            FMH::MODEL_NAME[FMH::MODEL_KEY::URL], url);
...
bool CollectionDB::updateTimeToDB(const QString &url, const int &trackOrVideo)
{
    QMutexLocker locker(&m_nMutext);
    auto queryTxt = QString("UPDATE %1 SET %2 = %3 WHERE %4 = \"%5\"").arg(TABLEMAP[TABLE::TRACKS],
            FMH::MODEL_NAME[FMH::MODEL_KEY::RELEASEDATE],
            QString::number(QDateTime::currentMSecsSinceEpoch()),
            FMH::MODEL_NAME[FMH::MODEL_KEY::URL], url);

    if(trackOrVideo == 2)
    {
        queryTxt = QString("UPDATE %1 SET %2 = %3 WHERE %4 = \"%5\"").arg(TABLEMAP[TABLE::VIDEOS],
                FMH::MODEL_NAME[FMH::MODEL_KEY::RELEASEDATE],
                QString::number(QDateTime::currentMSecsSinceEpoch()),
                FMH::MODEL_NAME[FMH::MODEL_KEY::URL], url);
    }
...
bool CollectionDB::removeVideo(const QString &path)
{
    auto queryTxt = QString("DELETE FROM %1 WHERE %2 =  \"%3\"").arg(TABLEMAP[TABLE::VIDEOS],
            FMH::MODEL_NAME[FMH::MODEL_KEY::URL],path);
    auto query = this->getQuery(queryTxt);
    if(query.exec())
    {
...
void CollectionDB::removeMissingVideos()
{
    auto videos = this->getDBData("select url from videos");
    for(auto video : videos)
        if(!BAE::fileExists(video[FMH::MODEL_KEY::URL]))
            this->removeVideo(video[FMH::MODEL_KEY::URL]);
}
