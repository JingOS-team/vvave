...
bool CollectionDB::playedTrack(const QString &url, const int &increment)
{
	...
	if(query.exec())
	{
		qDebug() << "CollectionDB playedTrack ok == " << url;
		return true;
	}
...
bool CollectionDB::playedVideo(const QString &url, const int &increment)
{
	...
	if(query.exec())
	{
		qDebug() << "CollectionDB playedVideo ok == " << url;
		return true;
	}
...
bool CollectionDB::updateTimeToDB(const QString &url, const int &trackOrVideo)
{
	auto queryTxt = QString("UPDATE %1 SET %2 = %3 WHERE %4 = \"%5\"").arg(TABLEMAP[TABLE::TRACKS],
			...
			FMH::MODEL_NAME[FMH::MODEL_KEY::URL], url);//默认是音频
		
	if(trackOrVideo == 2)//视频
	{
	...
	if(query.exec())
	{
		qDebug() << "update time == " << QDateTime::currentDateTime().toString(Qt::DateFormat::TextDate);
		return true;
	}
		
	qDebug() << "updateTimeToDB false";
	return false;
}
...
bool CollectionDB::removeVideo(const QString &path)
{
	...
	{
		if(cleanAlbums()) cleanArtists();
		qDebug()<< "hjy removeVideo ok";
		return true;
	}
	qDebug()<< "hjy removeVideo false";
	return false;
}
