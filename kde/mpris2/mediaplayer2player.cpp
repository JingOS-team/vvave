/***************************************************************************
   ...
   SPDX-FileCopyrightText: 2021 (c) Yu Jiashu <yujiashu@jingos.com>
...
MediaPlayer2Player::MediaPlayer2Player(TracksModel *playListControler, Player *audioPlayer, bool showProgressOnTaskBar, QObject *parent)
    ...
    , mShowProgressOnTaskBar(showProgressOnTaskBar)
{
    ...
    connect(m_playListControler, &TracksModel::currentIndexChanged, this, &MediaPlayer2Player::playerSourceChanged, Qt::QueuedConnection);
    connect(this, &MediaPlayer2Player::next, m_playListControler, &TracksModel::nextRequest, Qt::QueuedConnection);
    connect(this, &MediaPlayer2Player::previous, m_playListControler, &TracksModel::previousRequest, Qt::QueuedConnection);
...
{
    if (m_playListControler) {
        m_audioPlayer->setPlaying(false);
    }
}
...
void MediaPlayer2Player::PlayPause()
{
    ...
    if (m_playListControler) {
        if(m_audioPlayer->getState() == QMediaPlayer::State::PlayingState){
            m_audioPlayer->setPlaying(false);
        } else {
            m_audioPlayer->setPlaying(true);
        }
    }
}
...
void MediaPlayer2Player::setPropertyPosition(int newPositionInMs)
{
    ...
    if (mShowProgressOnTaskBar && (incrementalProgress > 0.01 || incrementalProgress < 0)) {
        ...
        mProgressIndicatorSignal.setArguments({QStringLiteral("application://org.kde.media.desktop"), parameters});
...
void MediaPlayer2Player::setCurrentTrack(int newTrackPosition)
{
    ...
    m_currentTrackId = QDBusObjectPath(QLatin1String("/org/jingos/media/playlist/") + QString::number(newTrackPosition)).path();
